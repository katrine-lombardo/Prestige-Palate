...
@router.get("/photos/place/{place_id}", response_model=List[PhotoOut])
async def get_photos_by_place(place_id: str):
    photoQueries = PhotoQueries()
    try:
        photo_urls = photoQueries.get_photos_by_place(place_id)
        if not photo_urls:
            raise HTTPException(
                status_code=404,
